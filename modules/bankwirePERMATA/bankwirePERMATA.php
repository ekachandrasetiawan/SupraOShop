...
class BankWirePERMATA extends PaymentModule
{
	private $_html = '';
	...
	public function __construct()
	{
		...
		$config = Configuration::getMultiple(array('PERMATA_DETAILS', 'PERMATA_OWNER', 'PERMATA_ADDRESS'));
		if (isset($config['PERMATA_OWNER']))
			$this->owner = $config['PERMATA_OWNER'];
		if (isset($config['PERMATA_DETAILS']))
			$this->details = $config['PERMATA_DETAILS'];
		if (isset($config['PERMATA_ADDRESS']))
			$this->address = $config['PERMATA_ADDRESS'];

		parent::__construct();

		$this->displayName = $this->l('Bank Wire PERMATA');
		$this->description = $this->l('Accept payments by PERMATA (bank wire)');
	...
	{
		if (!Configuration::deleteByName('PERMATA_DETAILS')
				OR !Configuration::deleteByName('PERMATA_OWNER')
				OR !Configuration::deleteByName('PERMATA_ADDRESS')
				OR !parent::uninstall())
			return false;
	}
	...
	private function _postProcess()
	{
		if (isset($_POST['btnSubmit']))
		{
			Configuration::updateValue('PERMATA_DETAILS', $_POST['details']);
			Configuration::updateValue('PERMATA_OWNER', $_POST['owner']);
			Configuration::updateValue('PERMATA_ADDRESS', $_POST['address']);
		}
	...
	{
		$this->_html .= '<img src="../modules/bankwirePERMATA/bankwire.jpg" style="float:left; margin-right:15px;"><b>'.$this->l('This module allows you to accept payments by bank wire.').'</b><br /><br />
